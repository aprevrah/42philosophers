...
int	parse_input(t_philo_sim *philo_sim, int argc, char **argv)
{
	...
	philo_sim->nbr_of_philos = ft_atoi(argv[1]);
	philo_sim->time_to_die = ft_atoi(argv[2]);
	philo_sim->time_to_eat = ft_atoi(argv[3]);
	philo_sim->time_to_sleep = ft_atoi(argv[4]);
	if (argc == 6)
	{
		philo_sim->nbr_of_times_each_philo_must_eat = ft_atoi(argv[5]);
		if (philo_sim->nbr_of_times_each_philo_must_eat < 1)
			return (printf("Invalid input\n"), 1);
	}
	else
		philo_sim->nbr_of_times_each_philo_must_eat = -1;
	if (philo_sim->nbr_of_philos < 1 || philo_sim->nbr_of_philos > 200)
...
pthread_mutex_t	*init_silverware(t_philo_sim *philo_sim)
{
	...
	forks = malloc(philo_sim->nbr_of_philos * sizeof(pthread_mutex_t));
	if (!forks)
		return (NULL);
	i = 0;
	while (i < philo_sim->nbr_of_philos)
	{
